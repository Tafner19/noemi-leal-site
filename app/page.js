...
export default function NoemiLealSite() {
  ...
  const servicos = [
    ...
    { 
      nome: "Mixagem e Masterização", 
      preco: "R$ 200", 
      icon: <Radio className="w-6 h-6 text-pink-500" /> 
    },
    { 
      nome: "Captação de Voz", 
      preco: "R$ 250", 
  ...
  const produtosDigitais = [
    {
      ...
      descricao: "Preset profissional para tratamento de voz"
    },
    {
      nome: "Preset de Back Vocals",
      preco: "R$ 14,90",
      icon: <Sliders className="w-6 h-6 text-pink-400" />,
      descricao: "Preset para harmonias e backing vocals"
    },
    {
      nome: "Preset de Master",
      preco: "R$ 29,90",
      icon: <ShoppingBag className="w-6 h-6 text-pink-400" />,
      descricao: "Preset de masterização profissional"
    }
  ];
  ...
  return (
    <div className="bg-[#0b0b0b] text-white min-h-screen font-sans antialiased selection:bg-pink-500 selection:text-black">
        ...
        <div className="max-w-7xl mx-auto px-6 w-full grid md:grid-cols-2 gap-12 items-center relative z-10">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="#servicos" className="bg-white text-black font-semibold px-8 py-4 rounded-full hover:bg-zinc-200 transition-all text-center">
                Ver Serviços
              </a>
              <a href="#portfolio" className="border border-zinc-700 text-zinc-300 font-semibold px-8 py-4 rounded-full hover:bg-zinc-900 transition-all text-center">
                Ver Portfólio
              </a>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="border-t border-zinc-800 pt-16">
            <div className="text-center mb-10">
              <span className="text-pink-500 text-sm font-semibold tracking-wider block mb-2">// LOJA DIGITAL</span>
              <h3 className="text-2xl md:text-3xl font-bold mb-2">Produtos Digitais</h3>
              <p className="text-zinc-400 text-sm max-w-md mx-auto">Presets profissionais para elevar sua produção. Download imediato após a compra.</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {produtosDigitais.map((produto, index) => (
                <div 
                  key={index}
                  className="bg-[#0b0b0b] border border-zinc-800/80 hover:border-pink-500/50 p-6 rounded-2xl transition-all duration-300 flex flex-col justify-between group"
                >
                  <div>
                    <div className="mb-4 w-10 h-10 bg-zinc-900 rounded-xl flex items-center justify-center border border-zinc-800 group-hover:border-pink-500/40 transition-colors">
                      {produto.icon}
                    </div>
                    <h4 className="text-lg font-bold mb-1">{produto.nome}</h4>
                    <p className="text-zinc-500 text-xs mb-4">{produto.descricao}</p>
                    <div className="text-2xl font-extrabold text-white">{produto.preco}</div>
                  </div>
                  <a 
                    href={`https://wa.me/5511969452584?text=Olá%20Noemi!%20Tenho%20interesse%20em%20adquirir%20o%20${encodeURIComponent(produto.nome)}.`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-6 block w-full text-center text-sm font-semibold py-3 rounded-xl bg-zinc-800 hover:bg-pink-600 text-zinc-200 hover:text-white transition-all duration-300"
                  >
                    Adquirir Preset
                  </a>
                </div>
      ...
      {/* ─── 3. PORTFÓLIO ─── */}
      <section id="portfolio" className="py-20 border-t border-zinc-900 bg-[#0e0e0e]">
      ...
      {/* ─── 4. SOBRE MIM ─── */}
      <section id="sobre" className="py-20 border-t border-zinc-900">
